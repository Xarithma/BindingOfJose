...
func _process(_delta: float) -> void:
	...
	# Hide, if it's too close to player.
	# I know this is just a bandade.
	# Proper fix will be in the editor update.
	if global_transform.origin.distance_to(_player_orig) < 2.075:
		hide()
	else:
		show()
	...
	_amount_phase_check(get_tree().get_nodes_in_group("Blub").size())
...
func _amount_phase_check(amount: int) -> void:
	if amount <= 0:
		_phase += 1
