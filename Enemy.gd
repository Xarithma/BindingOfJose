...
# Projectile, set if the enemy is a turret.
var _projectile


func _ready() -> void:
	if _type == ENEMY_TYPE.TURRET:
		_projectile = load("res://Projectile.tscn")
...
func _spawn_hit_indicator(_indicator_scene: String, body: Node):
	var _hit_indicator = load(_indicator_scene).instance()
	body.add_child(_hit_indicator)
...
func _on_Area_body_entered(body: Node) -> void:
	...
	match _type:
		ENEMY_TYPE.BLOB:
			body.slow_down()
			_spawn_hit_indicator("res://BlobSplash.tscn", body)
			return
	body.damage(_damage)
	_spawn_hit_indicator("res://DamageIndicator.tscn", body)
